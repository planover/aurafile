...
## [0.1.18] - 2026-07-15
- 关键修复：fpk/config/privilege 改为 run-as: root，解决容器以非特权用户运行导致 /data 大部分子目录无权限读取、索引漏扫 99.97%（仅 130/527873 文件）的问题
- 索引器不再静默吞掉 readdir 错误：权限不足/路径不存在/符号链环路等现在打印 [Aurafile] 索引跳过（原因）：路径 日志，便于定位漏扫
- 新增跨版本索引库检测：旧库无版本标记或版本落后时打印警告，提示清空索引并重建
...
### Added
- **目录名也可被搜索（Everything 式补全）**：此前索引器只对文件建库，文件夹名无法被搜到（用户投诉「52 万文件里按目录名搜不到」）。现索引器对每个目录也写一条记录（`isDir=1`），`db.search()` 的 `LIKE '%term%'` 子串匹配同时覆盖目录与文件，结果中**目录排在文件前**（与 Everything 一致）。watcher 同步覆盖 `addDir`/`unlinkDir`，新建/删除目录实时进/出索引。
- 前端：搜索结果中的文件夹以 📁 图标 + 青绿区分样式展示，点击直接进入该目录浏览（与浏览模式一致）；类型筛选下拉新增「文件夹」；统计条改为「已索引 N 项（文件 X · 文件夹 Y）」。
- 旧库兼容：升级用户的 `files` 表自动 `ALTER` 补齐 `isDir` 列，并把历史上 `kind='folder'` 的目录回填 `isDir=1`，旧库目录仍可被搜到。
- `/api/stats` 现返回 `{files, dirs, total}`。

### Notes
- 目录数单独预算（`SCAN_MAX_DIRS=300万`），不挤占文件扫描额度。
- 已知边界：指向目录的符号链接当前不会被递归/入库（不计入搜索）；纯子串 `LIKE` 为全表扫描，目录入库使扫描行数按比例略增，属既有设计非新瓶颈。

## [0.1.15] - 2026-07-14

### Fixed
- **搜索功能彻底失效（核心功能级）**：原 `db.search()` 使用 SQLite FTS5 `MATCH`，存在三处致命问题——① 它是「词匹配」而非「子串匹配」，导致 `alpine` 搜不到 `alpine-virt-3.23.3-x86_64.iso`；② `unicode61` 分词器完全不处理中文，`系统` 等中文词 0 结果；③ 用户输入含 FTS5 特殊字符（`"` `*` `(` 等）会直接抛 SQL 语法错。已重构为 **Everything 式文件名子串匹配**（`files.name LIKE '%term%'`，大小写不敏感、中英文通用），FTS5 全文内容搜索降级为可选增强（显式开启且做特殊字符清洗）。已通过确定性单元测试验证：中文子串、英文子串、大小写不敏感、通配符转义全部 PASS。
- **52 万文件「看不到」**：前端此前只走 `/api/timeline?limit=800`，默认仅显示最近修改的 800 个文件，造成「文件没被识别」的错觉；且 `SCAN_MAX_FILES` 上限 50 万，用户 527743 个文件被漏扫 27743 个。修复：① `SCAN_MAX_FILES` 提到 300 万；② 新增 **📁 浏览模式**（面包屑 + 目录树，直接钻进 `/data` 逐目录查看全部文件）；③ 顶栏实时显示「已索引 N 个文件」。

### Added
- `/api/search` 现返回 `total`（匹配总数）
- `/api/stats` 返回已索引文件数
- 前端「找到 N 个匹配结果」提示条
- 📁 浏览按钮与路径面包屑导航
